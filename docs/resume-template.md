...
```
姓名 | 联系方式 | GitHub: github.com/weifu1997
...
GitHub: github.com/weifu1997/multi-agent-commerce
...
• 基于LangGraph状态图实现两阶段并行(画像‖召回 → 重排‖库存 → 文案),
  提供/recommend/graph可视化接口,一套Python代码端到端跑通

技术栈: LangGraph · LangChain · Redis · Milvus · FastAPI
```
...
• 工程化交付: Docker一键部署(Redis/Milvus依赖容器化),
  LangGraph可视化编排 + structlog结构化日志,便于维护与演示
```
...
### 后端工程师
重点: 并行编排(asyncio + LangGraph)、
      Redis、FastAPI、Docker部署
